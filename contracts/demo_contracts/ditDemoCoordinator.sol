...
contract ditDemoCoordinator {
    ...
    function proposeCommit(bytes32 _repository, string calldata _description, string calldata _identifier, uint256 _knowledgeID, uint256 _numberOfKNW, uint256 _voteDuration, uint256 _amountOfTokens) external onlyPassedKYC(msg.sender) returns (uint256 proposalID) {
        ...
        proposalsOfRepository[_repository][newProposalID] = proposal({
            ...
            KNWVoteID: KNWVote.startVote(_repository, msg.sender, _knowledgeID, _voteDuration, _amountOfTokens, _numberOfKNW),
    ...
    function voteOnProposal(bytes32 _repository, uint256 _proposalID, bytes32 _voteHash, uint256 _numberOfKNW) external onlyPassedKYC(msg.sender) returns (bool) {
        ...
        emit CommitVote(_repository, _proposalID, msg.sender, proposalsOfRepository[_repository][_proposalID].knowledgeID, proposalsOfRepository[_repository][_proposalID].individualStake, _numberOfKNW, numberOfVotes);
